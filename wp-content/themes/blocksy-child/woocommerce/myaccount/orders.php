					...
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								echo '<div class="ct-header-cta" style="display:flex;justify-content:flex-end;align-items:flex-start;">';
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										echo '<a data-size="small" href="' . esc_url( $action['url'] ) . '" class="woocommerce-button ct-button ' . esc_attr( $wp_button_class ) . ' button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
									}
									if($file_download){
										echo '<a data-size="small" href="' . esc_url( $file_download ) . '" class="woocommerce-button ct-button ' . esc_attr( $wp_button_class ) . ' button ' . sanitize_html_class( 'download' ) . '">' . esc_html__( 'Download', 'woocommerce' ) . '</a>';
									}
								}
								echo '</div>';
								?>
...
<?php else : ?>
	...
	<?php wc_print_notice( 
		'No se ha solicitado ningun informe todavía'
	.' <a class="woocommerce-Button wc-forward button' . esc_attr( $wp_button_class ) . '" href="solicita-tu-informe">Pedir Informe</a>', 'notice'
	// .' <a class="woocommerce-Button wc-forward button' . esc_attr( $wp_button_class ) . '" href="' . esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ) . '">' . esc_html__( 'Browse products', 'woocommerce' ) . '</a>', 'notice'
